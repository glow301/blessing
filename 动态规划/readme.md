...
#### 01 背包
##### 问题描述
有 N 件物品和一个容量为 V 的背包，第 i 个物品的体积是 $C_i$，价值是 $W_i$，求解将哪些物品放入背包可以使总价值最大。
...
##### 优化
对于时间复杂度，已经没有办法再进行优化了，但是，空间复杂度还有优化的空间。我们可以发现，在第二层 for 循环中，dp[i][j] 只和上一次的结果有关，也就是 dp[i-1][*]。我们可以改变迭代的方向，来将二维数组压缩成一维数组。
